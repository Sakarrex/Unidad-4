...
class ContactForm(tk.LabelFrame):
    fields = ("Apellido", "Nombre", "Teléfono","Peso","Altura")
    def __init__(self, master, **kwargs):
    ...
    def mostrarEstadoContactoEnFormulario(self, contacto):
        ...
        values = (contacto.getApellido(), contacto.getNombre(),
                  contacto.getTelefono(),contacto.getPeso(),contacto.getAltura())
...
class IMCview(tk.Toplevel):
    def __init__(self,parent,IMC,Composicion):
        super().__init__(parent)
        self.resizable(0,0)
        tk.Label(self, text="IMC").grid(row=0, column=0)
        tk.Label(self, text="Composicion Corporal").grid(row=1, column=0)
        tk.Label(self,text="{:.2f}".format(IMC)).grid(row=0, column=1)
        tk.Label(self,text=Composicion).grid(row=1, column=1)
        tk.Button(self,text="Volver",command=self.destroy).grid(row=2,column=1,padx=10, pady=10)
    def show(self):
        self.grab_set()
        self.wait_window()
